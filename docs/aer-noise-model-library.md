...
`undated`, `future`, `aging`, `stale`, `wrong_backend`, `wrong_source`, and
`unmappable` return no model, so incomplete, future, mismatched, old, or
unsupported data cannot appear authoritative. The builder expects
`backend_name == "ibm_fez"` and `source == "backend-properties-api:v1"` by
default; callers may provide explicit expected values for another versioned
snapshot source.
...
The retained `seed` is provenance metadata for reproducibility records. The
current construction path is deterministic from the snapshot and does not use
the seed to generate randomness.